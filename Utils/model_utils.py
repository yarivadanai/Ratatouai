...
class RatBrain:
    ...
    @traceable
    def run_full_chain(self, images: List[str]):
        ...
        for img in images:
            answer = ImageAnswer.parse_raw(self._analyzeImage(img))
        ...
        recipes = self._getRecipes(ingredients)

        for recipe in recipes:
            recipe = recipe.json()

            missing_ingredients_prompt = self._get_prompt_template(prompts.system_ingredients_gaps_prompt_template, 
                                                            prompts.human_ingredients_gaps_prompt_templatee)
            reflection_prompt = self._get_prompt_template(prompts.system_recipe_reflection_prompt_templatee, 
                                                    prompts.human_recipe_reflection_prompt_template)
            formatted_recipe_prompt = self._get_prompt_template(prompts.system_content_formatting_prompt_template, 
    ...
    @traceable
    def _analyzeImage(self, img: str) -> ImageAnswer:
    ...
    def _format_ingredients(self, ingredients: str):
        ...
        result = self._execute_chain(prompts.system_content_formatting_prompt_template, 
                        prompts.human_ingredients_formatting_prompt_template, self.formatting_model, 
                        StrOutputParser(), {"ingredients": ingredients}, True)
        return result

    @traceable
    def _getRecipes(self, ingredients: str) -> RecipesLists:
        """Gets recipes based on the ingredients and returns a RecipesLists object."""
        recipes = self._execute_chain(prompts.system_ingredients2recipe_prompt_template, 
                        prompts.human_ingredients2recipe_prompt_template, self.reasoning_model, 
    ...
    @traceable 
    def _get_prompt_template(self, system_template, human_template):
    ...
    def _stream_jsonoutput_chain(self, chain: Callable, params_dict: dict) -> Generator:
        """Streams a chain of operations and yields the result."""
        initial_json={}
        try:
            for diff in chain.stream(params_dict):

                yield jsonpatch.apply_patch(initial_json, diff)
